...
RSpec.describe Admin::JobsController, type: :controller do
  ...
  describe 'PUT #update' do
    ...
    context 'admin can aproove job' do
      let(:job) { create(:job) }
      let(:admin) { create(:user, :admin) }

      before do
        login_user(admin)
        put 'aproove', params: { id: job.id }
      end

      it 'admin aproove job' do
        expect(job.reload.status).to eq 'aprooved'
      end
    end

    context 'admin can unaproove job' do
      let(:job) { create(:job, :aprooved) }
      let(:admin) { create(:user, :admin) }

      before do
        login_user(admin)
        put 'unaproove', params: { id: job.id }
      end

      it 'admin unaproove job' do
        expect(job.reload.status).to eq 'unaprooved'
      end
    end
  end
